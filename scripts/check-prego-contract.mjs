...
function parseArguments(args) {
  ...
  for (let index = 0; index < args.length; index += 1) {
    ...
    result[key] = key === 'openApi' && /^https?:\/\//.test(value)
      ? value
      : resolve(value)
    index += 1
  }
...
function recordsByTool(records) {
  ...
  for (const record of records) {
    if (result.has(record.tool)) throw new Error(`tool ${record.tool}이 중복됩니다`)
...
function backendAdapterEnums(backendRoot) {
  ...
  return readdirSync(directory)
    .filter(
      (file) => file.endsWith('McpReadAdapter.kt') && file !== 'PregoMcpReadAdapter.kt',
    )
    .map((file) => {
      const source = readFileSync(join(directory, file), 'utf8')
      const match = source.match(/override val tool: PregoMcpTool = PregoMcpTool\.([A-Z_]+)/)
      if (!match) throw new Error(`BE adapter ${file}에 PregoMcpTool 연결이 없습니다`)
      return match[1]
    })
    .sort()
}
...
function assertBackendRegistryAndHandlerDispatch(backendRoot) {
  const mcpDirectory = join(backendRoot, 'application/src/main/kotlin/good/hr/api/mcp')
  const registrySource = readFileSync(join(mcpDirectory, 'PregoMcpToolRegistry.kt'), 'utf8')
  const handlerSource = readFileSync(join(mcpDirectory, 'PregoMcpStatelessServer.kt'), 'utf8')
  ...
  assert.match(
    handlerSource,
    /val execution = toolRegistry\.call\(currentCompanyScopedUser\(\), toolName, arguments\)/,
...
export function checkPregoContract({
  ...
  assert.equal(contract.version, 1, '지원하지 않는 pilot contract version입니다')
  ...
  assertSameRecords('BE PregoMcpTool enum', contract.tools, backendEnumRecords(backendRoot))
  assert.deepEqual(
  ...
  for (const skill of contract.skills) {
    const actualTools = skillTools(join(PLUGIN_ROOT, 'skills', skill.id, 'SKILL.md'))
    assert.deepEqual(actualTools, [...skill.tools].sort(), `${skill.id}: 허용 tool이 contract와 다릅니다`)
    assert.ok(actualTools.every((tool) => approvedTools.has(tool)), `${skill.id}: 승인되지 않은 tool을 참조합니다`)
